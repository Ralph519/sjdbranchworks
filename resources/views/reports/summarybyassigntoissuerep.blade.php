        ...
        <div class="col-xs-12">
          <div class="cardsmall">
            <h2 class="invoice-header">
              <i class="fa fa-file-text-o"></i> Summary of Tickets created per Assign To and Issue Reported
              <br>
                ...
                <tbody>
                  ...
                  <tr>
                    @foreach($repitem as $column_name => $value)
                      ...
                      ?>
                      @if($column_name != 's_assignto' && $column_name != 'name')
                        <td><a href="{{ route('summaryByAssignedtoIssueRepDetail', ['assignto' => $repitem->s_assignto, 'assigntoname' => $repitem->name, 'date1' => $date1, 'date2' => $date2, 'reptype' => $lRepType, 'issuetype' => $lIssueType]) }}">{{$value}}</a></td>
                        <?php
